...
const resourceUrl = args[0];

if (!resourceUrl || resourceUrl === "--help" || resourceUrl === "-h") {
  console.log("Usage: bun run x402 <URL>");
  process.exit(resourceUrl ? 0 : 1);
}
...
async function main() {
  console.log(`[x402] Fetching ${resourceUrl}`);

  const initialResponse = await fetch(resourceUrl, { method: "GET" });
  ...
  await printResponse(paidResponse);
}
